...
namespace SmartMeetingRoom.CaptureApp
{
    internal class LocalFaceDetector : IImageFilter
    {
        ...
        public async Task ProcessImageAsync(Stream imageStream, string cameraId, string empName, string empDeptName)
        {
            await _imagePersiter.PersistAsync(imageStream, cameraId, empName, empDeptName);
        }
